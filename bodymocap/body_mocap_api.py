...
class BodyMocap(object):
    ...
    def regress(self, img_original, body_bbox_list, image_path):
        """
            args: 
        ...
        for body_bbox in body_bbox_list:
            ...
            with torch.no_grad():
                ...
                def filter_visible_vertices(pred_vertices_img, img_h, img_w):
                    front_vertices_mask = pred_vertices_img[:, 2] > 0
    
                    in_img_mask = (pred_vertices_img[:, 0] >= 0) & (pred_vertices_img[:, 0] < img_w) & \
                                (pred_vertices_img[:, 1] >= 0) & (pred_vertices_img[:, 1] < img_h)
                    
                    visible_mask = front_vertices_mask & in_img_mask
                    
                    visible_indices = np.where(visible_mask)[0].astype(np.int32)  
                    visible_xy = pred_vertices_img[visible_mask, :2]             
                    visible_xy_int = np.round(visible_xy).astype(np.int32)       

                    return visible_indices, visible_xy_int
                visible_vertex_indices, visible_vertex_xy_int = filter_visible_vertices(
                        pred_vertices_img, H, W
                    )
                save_data = np.column_stack([
                    visible_vertex_xy_int,
                    visible_vertex_indices      
                ])
                save_path = image_path.rsplit('.', 1)[0] + '.pkl'
                import pickle
                f = open(save_path, 'wb')
                pickle.dump(save_data, f)
                f.close()
                def sample_visible_faces(pred_vertices_img, visible_vertex_indices, smpl_faces, num_samples_per_face=2):

                    is_vertex_visible = np.zeros(pred_vertices_img.shape[0], dtype=bool)
                    is_vertex_visible[visible_vertex_indices] = True  
                    
                    visible_face_mask = is_vertex_visible[smpl_faces].all(axis=1)
                    visible_faces = smpl_faces[visible_face_mask]  # (F_vis, 3)
                    
                    if len(visible_faces) == 0:
                        print("Warning: No visible faces found!")
                        return np.empty((0, 8))
                    ...
                    for face in visible_faces:
                        ...
                        p0 = pred_vertices_img[v0, :2]
                        p1 = pred_vertices_img[v1, :2]
                        p2 = pred_vertices_img[v2, :2]
                        ...
                        for _ in range(num_samples_per_face):
                            ...
                            sample_xy = b0 * p0 + b1 * p1 + b2 * p2  
                            sample_xy_int = np.round(sample_xy).astype(np.int32) 
                            
                            sampled_data.append([
                                sample_xy_int[0], sample_xy_int[1],  
                                v0, v1, v2,                          
                                b0, b1, b2                             
                            ])
                    
                    return np.array(sampled_data, dtype=np.float32)
                
                sampled_data = sample_visible_faces(
                    pred_vertices_img, 
                    visible_vertex_indices, 
                    self.smpl.faces,  
                    num_samples_per_face=2 
                )

                sample_save_path = image_path.rsplit('.', 1)[0] + '_samples.pkl'
                f = open(sample_save_path, 'wb')
                ...
                #f = open('/data/frankmocap/pose.pkl', 'wb')
                #pickle.dump(pred_output['pred_body_pose'], f)
                pred_output['pred_rotmat'] = pred_rotmat.detach().cpu().numpy() # (1, 24, 3, 3)
                pred_output['pred_betas'] = pred_betas.detach().cpu().numpy() # (1, 10)
                #f = open('/data/frankmocap/shape.pkl', 'wb')
                #pickle.dump(pred_output['pred_betas'], f)
